...
limit=50000000 # Update the default limit to 50MB
limit=${GIT_FILE_SIZE_LIMIT:-50000000} # Update the default limit to 50MB
limitInMB=$(( $limit / 1000000 )) # Update the limit in megabytes
...
for file in $( git diff-index --cached --name-only $against ); do
	...
	if [ "$file_size" -gt  "$limit" ]; then
	    printError "File $file is $(( $file_size / 1000000 )) MB, which is larger than our configured limit of $limitInMB MB"
        shouldFail=true
	fi
done
if $shouldFail
