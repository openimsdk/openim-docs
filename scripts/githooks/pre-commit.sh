...
for file in $( git diff-index --cached --name-only $against ); do
    ...
    if [ $file_size -gt $limit ]; then
        printError "File $file is $filesize MB, which is larger than our configured limit of $limitInMB MB"
        shouldFail=true
    fi
shouldFail=false 
