...
for file in $( git diff-index --cached --name-only $against ); do
	...
	if [ "$file_size" -gt  "$limit" ] && { grep -q ".github/release-drafter.yml" $file; } && ! { git show HEAD:.github/release-drafter.yml; }
...
if [[ ! $local_branch =~ $valid_branch_regex ]] && ! { git show HEAD:.github/release-drafter.yml; }
then
