...
shouldFail=false
# Add a new function called `check_file_size_limit` after line 88
...
for file in $( git diff-index --cached --name-only $against ); do
        ...
        if [ $file_size -gt $2 ]; then
            shouldFail=true
            printError "File $file is $(($file_size / 2**20)) MB, which exceeds the maximum file size (2 MB) allowed by GitHub. The maximum file size allowed is 50MB. Commit aborted."
        fi
if $shouldFail
then
...
# Add a new function called `check_branch_name_format` after line 104

check_branch_name_format(){
    local valid_branch_regex="^(main|master|develop|release(-[a-zA-Z0-9._-]+)?)$|(feature|feat|openim|hotfix|test|bug|bot|refactor|revert|ci|cicd|style|)\/[a-z0-9._-]+\$^HEAD\$"
